...
public class TimeUtils {
    ...
    public String getDate(final Context context) {
        Calendar cal = Calendar.getInstance();
        String ret = DateFormat.format("yyyy-MM-dd-h-mm", cal).toString();
        return ret;
    }
    ...
    public static String[] getWeekDateString() {
     ...
     //   cal.add(Calendar.DAY_OF_MONTH, -1 * WEEK_COUNT);
        for (int i=0; i<WEEK_COUNT; i++) {
            ret[i] = DateFormat.format("MM-dd HH:mm", cal).toString();
            //cal.add(Calendar.DAY_OF_MONTH, 1);
        }

 /*
        db = dbHelper.getReadableDatabase();
        String sql = "select date from bpm";
        Cursor cursor = db.rawQuery(sql, null);
        String[] ret = new String[cursor.getCount()];
        for (int i = 0; i < cursor.getCount(); i++) {//row 값 받아오기
            ret[i] = String.valueOf(cursor.getString(0)); // 값 받아오는 부분
            Log.i("dasa", String.valueOf(ret[i]));
            cursor.moveToNext();//다음 row로 이동
        }
        cursor.close();
        db.close();
        */
